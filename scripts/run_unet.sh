...
if [ -f "outputs/models/unet/best_model.pt" ]; then
    ...
    python -c "
import json
with open('outputs/models/unet/training_log.json') as f:
    log = json.load(f)
...
"
    ...
    if [ -n "$GITHUB_TOKEN" ]; then
        echo ""
        echo "============================================="
        echo "  Pushing U-Net Results to GitHub"
        echo "============================================="
        ...
        git add outputs/models/unet/best_model.pt
        git add outputs/models/unet/norm_mean.npy
        git add outputs/models/unet/norm_std.npy
        git add outputs/models/unet/training_log.json
        git add outputs/results/unet.json 2>/dev/null || true

        git commit -m "add U-Net training results [SLURM job $SLURM_JOB_ID]" || echo "Nothing new to commit."
...
echo ""
echo "============================================="
echo "  U-Net job processing sequence complete."
echo "  End: $(date)"
echo "============================================="